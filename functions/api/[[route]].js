...
async function createEvent(request, env, db) {
  let body;
  try { body = await request.json(); } catch { return fail('リクエストの形式が正しくありません。'); }

  // 合言葉が設定されているときだけ確認する（未設定なら誰でも作成できる）
  const secret = env.CREATE_PASSWORD;
  if (secret) {
    if (!sameSecret(String(body.password ?? ''), secret)) {
...
/** 主催者が「回答者へのひとこと」だけを書き換える */
async function updateMemo(request, db, adminToken) {
  const ev = await db.prepare('SELECT id FROM events WHERE admin_token = ?').bind(adminToken).first();
  if (!ev) return fail('この日程調整は見つかりませんでした。', 404);
  ...
  const memo = clean(body.memo, 500);
  await db.prepare('UPDATE events SET memo = ? WHERE id = ?').bind(memo, ev.id).run();
  return json({ memo });
}
...
export async function onRequest(context) {
  ...
  try {
    if (m === 'POST' && seg.length === 1 && seg[0] === 'events') return await createEvent(request, env, db);
    ...
    if (m === 'PUT' && seg.length === 3 && seg[0] === 'a' && seg[2] === 'memo') return await updateMemo(request, db, seg[1]);
    ...
    if (m === 'GET' && seg.length === 1 && seg[0] === 'config') return json({ password_required: !!env.CREATE_PASSWORD });
